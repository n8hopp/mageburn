...
func add_item(player_name: String, score_value: String) -> void:
	...
	item.get_node("MarginContainer/HBoxContainer/Score").text = score_value
	item.offset_top = list_index * 100
	$"PanelContainer/Panel/Board/HighScores/ScoreItemContainer".add_child(item)


func add_no_scores_message() -> void:
	var item = $"PanelContainer/Panel/Board/MessageContainer/TextMessage"
	item.text = "No scores yet!"
	$"PanelContainer/Panel/Board/MessageContainer".show()
	item.offset_top = 135
...
func hide_message() -> void:
	$"PanelContainer/Panel/Board/MessageContainer".hide()


func clear_leaderboard() -> void:
	var score_item_container = $"PanelContainer/Panel/Board/HighScores/ScoreItemContainer"
